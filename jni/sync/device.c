...
*/
	...
	LOGI("opening socket, host %s", host);

	sa.sin_family = AF_INET;
	sa.sin_port = htons(nport);
    if (!inet_aton(host, &sa.sin_addr)) {
       	LOGE("cannot parse IP address: %s", host);
   	}
	...
	if (sock == INVALID_SOCKET) {
		...
		int conn=connect(sock, (struct sockaddr *)&sa, sizeof(sa));
		if (conn >= 0) {
			LOGI("connected to %s", host);
		} else {
	  		LOGE("connect failed: errno %i", errno);
			closesocket(sock);
			sock = INVALID_SOCKET;
		}
	}
...
static int load_track_data(struct sync_track *t, const char *path)
{
	...
	Assetloader al = loadasset((char *)path);
	char *data = al.data;
	memcpy(&t->num_keys, data, sizeof(size_t));
	data += sizeof(size_t);
...
/*		BEGIN MODS
		...
		*/
		...
		memcpy(&key->row, data, sizeof(size_t));
		data += sizeof(size_t);
		memcpy(&key->value, data, sizeof(float));
		data += sizeof(float);
